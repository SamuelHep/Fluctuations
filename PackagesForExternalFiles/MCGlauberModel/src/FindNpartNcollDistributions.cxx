...
void FindNpartNcollDistributions(NegBinomialSearchResults *glauberResult, TTree *glauberTree, bool useTrackingEfficiency){

  //Create the Structure that will be returned
  Int_t nCentBins = glauberResult->nCentralityBins;

  double hardness = glauberResult->hardness;
  ...
  TH1D * nbdHist = MakeNegativeBinomialHist(glauberResult->npp,glauberResult->k,glauberResult->hardness,0,*glauberEvent);
  ...
  for (Int_t iEvent=0; iEvent<glauberTree->GetEntries(); iEvent++){
    ...
    nParticles = glauberEvent->ProduceParticles(hardness,nbdHist,useTrackingEfficiency);
