...
void *LoopOverGlauberEvents(void *args){
  ...
  nbdHistoSysMutex.Lock();
  TH1D * nbdHist = MakeNegativeBinomialHist(argVals->npp,argVals->k,argVals->rHard,++gl_index,glauberEvent);
  nbdHistoSysMutex.UnLock();
  ...
  while (iEvent < argVals->nEvents && maxCalls<10000){
    maxCalls++;
    ...
    if (argVals->x < 0)
      nParticles = glauberEvent.ProduceParticles(nbdHist,argVals->useTrackingEfficiency);
    else
      nParticles = glauberEvent.ProduceParticles(argVals->x,nbdHist,argVals->useTrackingEfficiency);
...
void SystematicErrors(const Int_t nEvents, const Int_t nNucleonsA, const Int_t nNucleonsB,
  ...
  for (Int_t iThread=0; iThread < nThreads; iThread++){
    std::cout<< "Waiting for thread[" << iThread << "] to finish" << std::endl;
