...
void RunGlauberSimulation(Int_t nEvents, Int_t nNucleonsA, Int_t nNucleonsB,
			  Double_t nnCrossSection, Int_t model=0, TString outDir="data",int seed=-1){

  //Just in case the necessary code has not been loaded, load it
  gSystem->Load("../bin/GlauberClass_cxx");
  gSystem->Load("../bin/GlauberModel_cxx");

  //Run the Glauber Simulation
  GlauberModel(nEvents,nNucleonsA,nNucleonsB,nnCrossSection,model,outDir,seed);
