...
TOFEfficiencyMaker::TOFEfficiencyMaker() : MAXFXT3(200), PTBINS(200), ETABINS(460), mass(0.938272)
{
 ...
 std::vector<TString> h_names = {"match_nhf_10","match_nhf_12","match_nhf_15","match_m2_plus0.05","match_m2_minus0.05","all_nhf_10","all_nhf_12","all_nhf_15","all_dca_2","all_dca_2p5","all_dca_2p75","match_dca_2","match_dca_2p5","match_dca_2p75"};
				    ...
				    )
{
  ...
  for (int iEntry=0;iEntry<entries;iEntry++)
      ...
      for (int iTrack=0;iTrack<event->GetEntries();iTrack++)
	{
	  ...
	  bool dcaGood[3] = {false,false,false};
	  if ( dca < 2) dcaGood[0] = true;  
	  if ( dca < 2.5) dcaGood[1] = true;  
	  if ( dca < 2.75) dcaGood[2] = true;  
	  ...
	  if ( dcaGood[0] ) TOFHists[ "all_dca_2" ]->Fill( fxt3, pt, eta ); 
	  if ( dcaGood[1] ) TOFHists[ "all_dca_2p5" ]->Fill( fxt3, pt, eta ); 
	  if ( dcaGood[2] ) TOFHists[ "all_dca_2p75" ]->Fill( fxt3, pt, eta ); 
	  ...
	  if ( tofmatch[0] )
	    {
	      if ( dcaGood[0] ) TOFHists[ "match_dca_2" ]->Fill( fxt3, pt, eta ); 
	      if ( dcaGood[1] ) TOFHists[ "match_dca_2p5" ]->Fill( fxt3, pt, eta ); 
	      if ( dcaGood[2] ) TOFHists[ "match_dca_2p75" ]->Fill( fxt3, pt, eta ); 
	    }
...
void TOFEfficiencyMaker::StudyTOFEfficiency(TString filename,TString outfilename)
{
  ...
  std::vector<TGraph*> interp_graph_vec;
  TGraphSmooth * tgs = new TGraphSmooth();
  ...
  for ( auto &pair_iter : match_all_pair )
    {
      ...
      std::vector< TGraph* > ig_vec(h2_all_bins,(TGraph*) NULL);
      ...
      for (int i=0;i<gr_vec.size();i++)
	{
	  ...
	  gr_vec[i] = new TGraphAsymmErrors(pass,total,"cp");
	  gr_vec[i]->SetName(TString::Format("ratioGraph_%s_%i",ratioName.Data(),i ));
	  f_vec[i] = new TF1(TString::Format("ratioFunc_%s_%i",ratioName.Data(),i ),par,0.4,2.0);

	  double etaLow  = h2_match->GetXaxis()->GetBinLowEdge(i);
	  double etaHigh = h2_match->GetXaxis()->GetBinLowEdge(i) + h2_match->GetXaxis()->GetBinWidth(i);

	  TString etaName = TString::Format("#eta=[%.3f,%.3f]", etaLow, etaHigh); 
	  cout << etaName << endl;

	  TGraph * tg = tgs->Approx( gr_vec[i] , "linear",1000);
	  ig_vec[i] = (TGraph*) tg->Clone();
	  ig_vec[i]->SetName( TString::Format("interpolate_Graph_%s_%i",ratioName.Data(),i ));
	  ig_vec[i]->SetTitle( etaName );
	  gr_vec[i]->SetTitle( etaName );

	  cout << "FIT NAME: " << gr_vec[i]->GetName() << endl;
	  //	  gr_vec[i]->Fit( f_vec[i], "R");
	}
      
      std::vector<TCanvas*> canvases = PlotSlices( ig_vec, gr_vec, ratioName);
      ...
      interp_graph_vec.insert( interp_graph_vec.end(), ig_vec.begin(),ig_vec.end() );
    }
  ...
  outfile->cd("..");
  outfile->mkdir("Interp");
  outfile->cd("Interp");
  for ( auto &h : interp_graph_vec )
    {
      if ( h ) h->Write();
    }
...
std::vector<TCanvas*> TOFEfficiencyMaker::PlotSlices(std::vector<TGraph*> &f, std::vector<TGraphAsymmErrors*> &gr, TString canName)
{
  ...
  for ( int i=2;i<f.size();i++)
	...
	{ 
	  ...
	  graph->GetXaxis()->SetLimits(0.001,2.45);
      ...
      else
	{
      ...
      //      text->DrawLatexNDC(0.35,0.35, fit->GetTitle() );
